...
// ==========================================
// HALAMAN UTAMA SUB UJIAN TAJWID AL-FATIHAH
// ==========================================
export default function SubUjianTajwid() {
  ...
  // 💡 FUNGSI DIRECT GEMINI (50 SOAL UJIAN TAJWID SURAH AL-FATIHAH)
  const ambilSoalLangsungDariAI = async () => {
    try {
      setLoadingAI(true);
      const finalSumberData = sumber_data ? String(sumber_data) : "TJ_AL_FATIHAH";

      // Meminta 50 Butir Soal Ujian Tajwid Al-Fatihah dari Gemini API
  ...
  const navigasiKeReview = async () => {
    try {
      ...
      router.push({
        ...
        params: { 
          judul_bab: String(judul_bab || "Ujian Tajwid Surah Al-Fatihah"), 
          tipe_sumber: tipe_sumber ? String(tipe_sumber) : "text", 
          sumber_data: sumber_data ? String(sumber_data) : "TJ_AL_FATIHAH" 
        },
      });
    } catch (e) {
  ...
  if (loadingSession || loadingAI) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.teksLoading}>Sedang meracik 50 butir soal Ujian Tajwid Surah Al-Fatihah...</Text>
      </View>
    );
  }
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <View style={styles.mainContent}>
        <TouchableOpacity style={styles.backButton} onPress={() => handleBukaModalKeluar("back")}>
          ...
          <Text style={styles.backButtonText}>Kembali ke Ujian</Text>
        </TouchableOpacity>

        <Text style={styles.sectionTitle}>{judul_bab || "Ujian Tajwid Surah Al-Fatihah"}</Text>
      ...
      <Modal visible={showFinishedModal} transparent={true} statusBarTranslucent={true} animationType="none">
        <View style={styles.modalOverlay}>
          <Animated.View style={[styles.modalContent, { opacity: fadeAnim, transform: [{ scale: scaleAnim }] }]}>
            ...
            <Text style={styles.modalTitle}>Ujian Selesai!</Text>
            <Text style={styles.modalSubtitle}>Selamat, kamu telah menyelesaikan ujian latihan variasi 50 soal ini kawan.</Text>
            ...
            <TouchableOpacity style={styles.modalButtonSekunder} onPress={() => { setShowFinishedModal(false); fadeAnim.setValue(0); scaleAnim.setValue(0.8); router.back(); }}>
              <Text style={styles.modalButtonTextSekunder}>Kembali ke Ujian</Text>
            </TouchableOpacity>
          </Animated.View>
        </View>
      </Modal>
      ...
      <Modal visible={showExitModal} transparent={true} statusBarTranslucent={true} animationType="none">
        <View style={styles.modalOverlay}>
          <Animated.View style={[styles.modalContent, { opacity: fadeExitAnim, transform: [{ scale: scaleExitAnim }] }]}>
            ...
            <Text style={styles.modalTitle}>Meninggalkan Ujian?</Text>
            <Text style={styles.modalSubtitle}>Apakah kamu yakin ingin keluar? Seluruh progres ujian saat ini akan hilang.</Text>
            <TouchableOpacity style={styles.modalButtonUtama} onPress={handleTutupModalKeluar}>
              <Text style={styles.modalButtonText}>Tidak, Lanjutkan Ujian</Text>
            </TouchableOpacity>
