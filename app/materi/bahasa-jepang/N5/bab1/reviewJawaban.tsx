...
export default function ReviewJawabanKuis() {
  ...
  // --- STATE MODAL PERINGATAN KELUAR KHUSUS SIDEBAR & ANIMASI ---
  ...
  // 💡 Modal Peringatan khusus dipicu dari Menu Navigasi Sidebar
  const handleBukaModalKeluar = (target: "dashboard" | "profile" | "pengaturan") => {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={["top"]}>
      ...
      <View style={styles.mainContent}>
        ...
        <View style={[styles.kartuSoal, { backgroundColor: colors.card, borderColor: colors.border }, wajibRTL && [styles.kartuSoalArab, { backgroundColor: colors.isDark ? "#1E293B" : "#F8FAFF" }]]}>
          <View style={styles.rowTipeReviewBadge}>
            ...
            <View style={[styles.badgeMiniTipe, { backgroundColor: colors.isDark ? "#334155" : "#F1F5F9" }]}><Text style={[styles.txtMiniTipe, { color: colors.subtext }]}>{jenisTipe.toUpperCase()}</Text></View>
          </View>
        ...
        <View style={styles.navigasiBawah}>
          ...
          {indeksAktif === listSoal.length - 1 ? (
            /* Tombol Selesai Review: Langsung router.back() tanpa modal */
            <TouchableOpacity
      ...
      {/* ==================== SIDEBAR ==================== */}
      {/* 💡 Hanya navigasi menu Sidebar yang memicu Modal Peringatan */}
      <Sidebar
        ...
        <View style={styles.modalOverlay}>
          <Animated.View style={[styles.modalContent, { backgroundColor: colors.card }, { opacity: fadeExitAnim, transform: [{ scale: scaleExitAnim }] }]}>
            ...
            <Text style={[styles.modalTitle, { color: colors.text }]}>
              {language === "id" ? "Meninggalkan Halaman Review?" : "Leave Review Page?"}
            </Text>
...
const styles = StyleSheet.create({
  ...
  badgeMiniTipe: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  txtMiniTipe: { fontSize: 9, fontWeight: 'bold' },
