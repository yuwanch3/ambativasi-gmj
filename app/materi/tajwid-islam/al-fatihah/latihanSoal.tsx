...
// 💡 IMPORT DIRECT GEMINI SERVICE (Akses langsung Gemini AI)
...
// ==========================================
// HALAMAN UTAMA SUB UJIAN KUIS (TAJWID)
// ==========================================
export default function SubUjianNihongo() {
  ...
  // 💡 FUNGSI DIRECT GEMINI DENGAN FALLBACK TAJWID AL-FATIHAH
  const ambilSoalLangsungDariAI = async () => {
    try {
      setLoadingAI(true);
      const finalSumberData = sumber_data ? String(sumber_data) : "TJ_AL_FATIHAH";
      
      // Minta 10 soal latihan Tajwid Al-Fatihah
      const dataSoal = await generateSoalDirectGemini(finalSumberData, 10);
      setListSoal(dataSoal);
    } catch (error: any) {
      Alert.alert("Error Gemini", error.message || "Gagal memuat soal kuis Tajwid.");
    } finally {
      setLoadingAI(false);
    }
  };
  ...
  const navigasiKeReview = async () => {
    try {
      ...
      router.push({
        ...
        params: { 
          judul_bab: String(judul_bab || (language === "id" ? "Tajwid - Surah Al-Fatihah" : "Tajweed - Surah Al-Fatihah")), 
          tipe_sumber: String(tipe_sumber || "tajwid"), 
          sumber_data: String(sumber_data || "TJ_AL_FATIHAH") 
        },
      });
    } catch (e) {
    ...
    return (
      <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.teksLoading, { color: colors.subtext }]}>
          {language === "id"
            ? "Sedang meracik 10 butir soal Tajwid Al-Fatihah via AI..."
            : "Generating 10 practice questions for Tajweed Al-Fatihah via AI..."}
        </Text>
      </View>
    );
  }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={["top"]}>
      ...
      <View style={styles.mainContent}>
        ...
        <Text style={[styles.sectionTitle, { color: colors.text }]}>
          {judul_bab || (language === "id" ? "Tajwid - Surah Al-Fatihah" : "Tajweed - Surah Al-Fatihah")}
        </Text>
...
const styles = StyleSheet.create({
  ...
  // COMPONENT WRAPPERS VARIANT SOAL
